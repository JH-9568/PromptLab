...
exports.validateCreateWorkspace = [
  body('kind').isIn(['team']).withMessage('Only "team" kind workspaces can be created via this endpoint.'),
  body('name')
    .isString()
    .trim()
...
exports.validateSendInvite = [
  body('email').isEmail().withMessage('Please provide a valid email for invitation.'),
  body('role')
    .isIn(['viewer', 'editor', 'admin'])
    .withMessage('Role must be one of viewer, editor, or admin.'),
  handleValidationErrors,
...
exports.validateInviteToken = [
  param('token').isString().notEmpty().withMessage('Invitation token is required.'),
  handleValidationErrors,
];
...
exports.validatePagination = [
  ...
  query('sort')
    .optional()
    .isIn(['recent', 'name'])
    .withMessage('Sort can only be "recent" or "name".'),
  handleValidationErrors,
];
...
exports.validateWorkspacePromptCreate = [
  ...
  body('model_setting').isObject().withMessage('Model setting is required.'),
  body('model_setting.ai_model_id').isInt().withMessage('ai_model_id is required.'),
  body('role')
    .optional()
    .isIn(['viewer', 'editor'])
    .withMessage('Prompt role must be either viewer or editor.'),
