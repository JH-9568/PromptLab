...
// POST /workspaces/:id/invites (초대 발송 + 즉시 멤버 추가)
...
// GET /workspaces/:id/invites (초대 목록 - 이제 accepted 기록만 반환)
...
/**
 * 아래 세 개의 초대 엔드포인트는 더 이상 사용하지 않습니다.
 * 초대는 생성 시점에 자동으로 수락(멤버 추가)되므로,
 * 토큰 기반 accept / reject / cancel 플로우는 비활성화됩니다.
 */

// PATCH /workspaces/invites/:token/accept (초대 수락 - 비활성화)
exports.acceptInvite = async (req, res, next) => {
  return next(
    new BadRequestError(
      'INVITE_FLOW_DISABLED',
      'Workspace invites are auto-accepted when sent. This endpoint is disabled.'
    )
  );
};

// PATCH /workspaces/invites/:token/reject (초대 거절 - 비활성화)
...
// DELETE /workspaces/invites/:token (초대 취소 - 비활성화)
...
exports.createPromptInWorkspace = async (req, res, next) => {
  try {
    ...
    const result = await workspaceService.createPromptInWorkspace(workspaceId, userId, req.body);
